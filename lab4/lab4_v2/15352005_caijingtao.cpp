...
void random_forest(){
	...
	train.assign(Train.begin(),Train.end()) ;
	
	for ( int cnt=0 ; cnt<TreeNum ; cnt++ ){
		vector<vector<int> >().swap(Train) ;
		
		int Size = train.size() ;
		for ( int i=0 ; i<Size ; i++ )
			Train.push_back(train[rand()%Size]) ;
		
		Attribute.clear() ;
		
		for ( int i=0 ; i<3 ; i++ ){
			while ( 1 ){
				int attr = rand()%attrCnt ;
				if ( Attribute.count(attr)!=1 ){
...
//		sum += ans ;
//	}
//	cout << "avg: " << (double)sum/(double)100 ; 
...
void predict(const vector<vector<int> > &data){
	...
	for ( int i=0 ; i<Size ; i++ ){
		...
		if ( posi > nega ) Predict.push_back(1) ;
